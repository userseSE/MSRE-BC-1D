...
std::vector<std::vector<double>> HX2(std::vector<std::vector<double>> &y_hx2, 
    ...
    if (step == 0) {
        std::copy(hx2_initial_conditions[0].begin(), hx2_initial_conditions[0].end(), u.begin());
        std::copy(hx2_initial_conditions[1].begin(), hx2_initial_conditions[1].end(), v.begin());
    } else {
        std::copy(y_hx2[0].begin(), y_hx2[0].end(), u.begin());
        std::copy(y_hx2[1].begin(), y_hx2[1].end(), v.begin());
    }
    u.back() = Ts_HX2_L;
    v.front() = Tss_HX2_0;

    // Define the ODE system for the second heat exchanger model compatible with odeint
    auto pde_to_ode_hx2 = [&](const std::vector<std::vector<double>> &y, 
    ...
    if (step == 0) {
        std::copy(hx2_initial_conditions[0].begin(), hx2_initial_conditions[0].end(), y0[0].begin());
        std::copy(hx2_initial_conditions[1].begin(), hx2_initial_conditions[1].end(), y0[1].begin());
    } else {
        y0 = y_hx2;
    }
