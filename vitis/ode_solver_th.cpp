...
public:
  ...
  void solve(const OdeFuncPointer_th ode_func, float y[length_th], int step,
             Param_Thermal &params, const float q_prime[N], float t0 = 0.0, float t1 = 1.0) {
    float t = t0;
...
void calculateNorm(const float arr[length_th], float &norm) {
  ...
  void rkf45_step_th(const OdeFuncPointer_th ode_func, float t,
    ...
    // Compute the stages
    float k1[length_th], k2[length_th], k3[length_th], k4[length_th], k5[length_th], k6[length_th];
