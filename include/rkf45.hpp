...
void rk45_step(Func&& f, std::vector<double>& y, double& t, double& dt) {
    ...
    // 计算k1
    f(y, k1, t);  // Corrected argument order
    for (size_t i = 0; i < N; ++i) {
        y_temp[i] = y[i] + dt * b21 * k1[i];
    }

    // 计算k2
    f(y_temp, k2, t + a2 * dt);  // Corrected argument order
    for (size_t i = 0; i < N; ++i) {
        y_temp[i] = y[i] + dt * (b31 * k1[i] + b32 * k2[i]);
    }

    // 计算k3
    f(y_temp, k3, t + a3 * dt);  // Corrected argument order
    for (size_t i = 0; i < N; ++i) {
        y_temp[i] = y[i] + dt * (b41 * k1[i] + b42 * k2[i] + b43 * k3[i]);
    }

    // 计算k4
    f(y_temp, k4, t + a4 * dt);  // Corrected argument order
    for (size_t i = 0; i < N; ++i) {
        y_temp[i] = y[i] + dt * (b51 * k1[i] + b52 * k2[i] + b53 * k3[i] + b54 * k4[i]);
    }

    // 计算k5
    f(y_temp, k5, t + a5 * dt);  // Corrected argument order
    for (size_t i = 0; i < N; ++i) {
        y_temp[i] = y[i] + dt * (b61 * k1[i] + b62 * k2[i] + b63 * k3[i] + b64 * k4[i] + b65 * k5[i]);
    }

    // 计算k6
    f(y_temp, k6, t + a6 * dt);  // Corrected argument order
