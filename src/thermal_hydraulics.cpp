...
std::vector<double> thermal_hydraulics(std::vector<double>& y_th, 
    ...
    auto pde_to_ode_th = [&](double t, const std::vector<double>& y) {
        ...
        VectorXd temperature_fuel_dt = a_th * (AT * temperature_fuel) + b_th * (temperature_graphite - temperature_fuel) + d_th * VectorXd::Map(q_prime.data(), N).transpose();
        VectorXd temperature_graphite_dt = c_th * (temperature_fuel - temperature_graphite) + e_th * VectorXd::Map(q_prime.data(), N).transpose();
