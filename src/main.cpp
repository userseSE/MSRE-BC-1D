...
int main() {
    ...
    for (int step = 0; step < time_span; ++step) {
        ...
        std::vector<double> phi(y_n.begin(), y_n.begin() + N);
        std::vector<double> ci(y_n.begin() + N, y_n.end());
        ...
        std::vector<double> temperature_fuel(y_th.begin(), y_th.begin() + N);
        std::vector<double> temperature_graphite(y_th.begin() + N, y_th.end());
        ...
        std::vector<double> Ts_HX1(y_hx1.begin(), y_hx1.begin() + Nx);
        std::vector<double> Tss_HX1(y_hx1.begin() + Nx, y_hx1.end());
        ...
        std::vector<double> Tss_HX2(y_hx2.begin(), y_hx2.begin() + Nx);
        std::vector<double> Tsss_HX2(y_hx2.begin() + Nx, y_hx2.end());
    ...
    // Plotting
    plot_results(rho_matrix, phi_middle_matrix, ci_middle_matrix, temperature_fuel_middle_matrix);
