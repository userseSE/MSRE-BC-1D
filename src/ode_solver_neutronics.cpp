...
private:
  ...
  // }
  void compute_norm(double arr[length_neutr], double &norm) {
    norm = 0.0;  // Initialize norm to zero
    for (int i = 0; i < length_neutr; ++i) {
        norm += arr[i] * arr[i];
    }
    if (norm < 0.0) {
        norm = -1.0;  // This shouldn't really happen as squares are non-negative
    } else {
        norm = std::sqrt(norm);  // Directly compute square root
    }
  }
...
void ode_solver_neutr(double y[length_neutr], OdeFuncPointer ode_func, int step,
  ...
  std::cout << "Neutronics solver called" << std::endl;
